...
<body>
  <div class="transaction-container">
      ...
      @foreach($items as $item)
        <div class="transaction-item">
          <div class="transaction-item-info">
            ...
            <div class="transaction-item-details">Rp {{ number_format($item['price'], 0, ',', '.') }}</div>
          </div>
          <div class="transaction-item-qty">
            <div class="transaction-item-qty-label">x{{ $item['qty'] }}</div>
          </div>
          <div class="transaction-item-price">Rp {{ number_format($item['total'], 0, ',', '.') }}</div>
        </div>
      @endforeach
    </div>
    ...
    <form action="{{ route('penjualan.confirm') }}" method="POST" id="confirmForm">
      @csrf
      ...
      <div class="transaction-summary" style="margin-bottom: 10px;">
        ...
        <div class="summary-row">
          ...
          <span class="summary-value" id="serviceChargeDisplay">Rp {{ number_format($serviceCharge, 0, ',', '.') }}</span>
        </div>
        <div class="summary-row tax">
          <span class="summary-label">Tax (2.5%)</span>
          <span class="summary-value" id="taxDisplay">Rp {{ number_format($tax, 0, ',', '.') }}</span>
        </div>
        <div class="summary-row total">
          <span class="summary-label">Total</span>
          <span class="summary-value" id="totalDisplay">Rp {{ number_format($total, 0, ',', '.') }}</span>
        </div>
      </div>
      ...
      @foreach($items as $index => $item)
        <input type="hidden" name="items[{{ $index }}][ProdukID]" value="{{ $item['id'] }}">
        <input type="hidden" name="items[{{ $index }}][JumlahProduk]" value="{{ $item['qty'] }}">
      @endforeach

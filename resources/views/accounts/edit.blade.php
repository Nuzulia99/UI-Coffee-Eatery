        ...
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']" aria-current="page">Edit Akun</li>
            </ol>
            <div class="flex flex-col gap-2">
              <h6 class="mb-0 font-bold text-white capitalize">Edit Akun</h6>
              <a href="{{ route('accounts.index') }}" class="inline-flex items-center w-fit px-3 py-1 bg-white text-slate-700 font-semibold rounded-lg hover:bg-gray-100 transition text-sm" title="Kembali">
      ...
      <div class="w-full px-6 py-6 mx-auto">
        <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="capitalize dark:text-white">Form Edit Akun</h6>
            <p class="text-sm text-slate-500 dark:text-slate-400">Edit informasi akun pengguna</p>
          </div>
          <div class="p-6">
            @if ($errors->any())
            ...
            <form action="{{ route('accounts.update', $user) }}" method="POST" enctype="multipart/form-data">
              ...
              <!-- Profile Photo -->
              <div class="mb-6">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-3">Foto Profil</label>
                <div class="flex gap-6">
                  <div>
                    @if($user->profile_photo)
                      <img src="{{ asset($user->profile_photo) }}" alt="{{ $user->name }}" class="h-32 w-32 object-cover rounded-lg border-2 border-slate-300">
                    @else
                      <div class="h-32 w-32 bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center rounded-lg border-2 border-slate-300">
                        <i class="fas fa-user text-3xl text-white"></i>
                      </div>
                    @endif
                  </div>
                  <div class="flex-1">
                    <input type="file" name="profile_photo" accept="image/*" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600">
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">Format: JPG, PNG, GIF (Max: 2MB)</p>
                  </div>
                </div>
              </div>
              ...
              <div class="mb-4">
                ...
                <input type="text" name="username" value="{{ $user->username }}" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
                @error('username')
              ...
              <div class="mb-4">
                ...
                <input type="email" value="{{ $user->email }}" class="w-full px-4 py-2 border border-slate-300 rounded-lg bg-slate-100 dark:bg-slate-700 dark:text-slate-300 cursor-not-allowed" readonly>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Email tidak dapat diubah</p>
              </div>

              <!-- Role / Level -->
              <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-2">Role / Level</label>
                <select name="level" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
              ...
              <div class="flex gap-2 mt-8">
                ...
                <a href="{{ route('accounts.show', $user) }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-6 rounded-lg transition">

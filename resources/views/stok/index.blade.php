                ...
                <tr data-kategori="{{ $item->Kategori }}">
                  ...
                  <td class="px-6 py-3 border-b border-coloured border-solid dark:border-white/30 text-center">
                    <div class="flex flex-wrap items-center justify-center" style="gap: 0.2cm;">
                      <a href="{{ route('produk.edit', $item->ProdukID) }}" class="inline-flex items-center px-4 py-2 text-xs font-semibold text-white rounded-lg transition-colors hover:opacity-80" style="background-color: #3d5a73; gap: 0.5rem;" title="Edit">
                        <span style="font-size: 1.2rem;">✎</span>
                        <span>Edit</span>
                      </a>
                      <button type="button" onclick="openDeleteModal({{ $item->ProdukID }})" class="inline-flex items-center px-4 py-2 text-xs font-semibold text-white rounded-lg transition-colors hover:opacity-80" style="background-color: #f87171; gap: 0.5rem; border: none; cursor: pointer;" title="Hapus">
                        <span style="font-size: 1.2rem;">×</span>
                        <span>Hapus</span>
                      </button>
                    </div>
    ...
    <script>
      ...
      function confirmDelete() {
        ...
        if (deleteProductId) {
          ...
          form.method = 'POST';
          form.action = '{{ route("produk.destroy", ":id") }}'.replace(':id', deleteProductId);

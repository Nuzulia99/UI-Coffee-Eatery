...
<html>
  ...
  <title>{{ ucfirst($kategori) }} - Data Produk</title>
  ...
  <body class="m-0 font-sans text-base antialiased font-normal dark:bg-slate-900 leading-default text-slate-500" style="background: #3d5a73; min-height: 100vh;">
    ...
    <main class="relative h-full transition-all duration-200 ease-in-out xl:ml-68 rounded-xl">
        ...
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']" aria-current="page">{{ ucfirst($kategori) }}</li>
            </ol>
            <h6 class="mb-0 font-bold text-white capitalize">{{ ucfirst($kategori) }}</h6>
          </nav>
        </div>
      </nav>
      ...
      <div class="w-full px-6 py-6 mx-auto">
        ...
        <!-- Back Button -->
        <div class="mb-6">
          <a href="{{ route('produk.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg inline-block">
        ...
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border" style="outline: 2px solid #3d5a73; outline-offset: -8px;">
          <div class="p-6">
            <form method="GET" action="{{ route('produk.kategori', $kategori) }}" class="flex gap-2">
              <div class="flex-1">
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari produk..." class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 dark:bg-slate-800 dark:text-white dark:border-slate-600" style="focus-ring-color: #3d5a73;">
              </div>
              <button type="submit" class="text-white font-bold py-2 px-4 rounded-lg" style="background-color: #3d5a73;">
                <i class="fas fa-search"></i> Cari
              </button>
              @if ($search)
                <a href="{{ route('produk.kategori', $kategori) }}" class="text-white font-bold py-2 px-4 rounded-lg" style="background-color: #3d5a73;">
        ...
        <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border" style="outline: 2px solid #3d5a73; outline-offset: -8px;">
          <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="capitalize dark:text-white">Daftar Produk {{ ucfirst($kategori) }}</h6>
            @if ($search)
              <p class="text-sm text-slate-400 dark:text-white">Hasil pencarian: "{{ $search }}"</p>
            @endif
          </div>
            ...
            <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40">
              <thead class="align-bottom">
                <tr>
                  ...
                  <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-coloured border-solid text-xxs opacity-70 text-slate-400 dark:opacity-80 dark:border-white/30">Nama Produk</th>
                  ...
                  <th class="px-6 py-3 font-bold text-center uppercase align-middle bg-transparent border-b border-coloured border-solid text-xxs opacity-70 text-slate-400 dark:opacity-80 dark:border-white/30">Penjualan Hari Ini</th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr>
                  ...
                  <td class="px-6 py-3 border-b border-coloured border-solid dark:border-white/30 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white">{{ $item->NamaProduk }}</p>
                  </td>
                  <td class="px-6 py-3 border-b border-coloured border-solid dark:border-white/30 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white">Rp {{ number_format($item->Harga, 0, ',', '.') }}</p>
                  </td>
                  <td class="px-6 py-3 border-b border-coloured border-solid dark:border-white/30 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white">{{ $item->Stok }}</p>
                  </td>
                  <td class="px-6 py-3 border-b border-coloured border-solid dark:border-white/30 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white">{{ $item->penjualan_hari_ini ?? 0 }}</p>
                  </td>
                </tr>
                @empty
                <tr>
                  <td colspan="5" class="px-6 py-3 text-center text-sm font-semibold text-slate-400 dark:text-white">
                    @if ($search)
                      Tidak ada produk yang sesuai dengan pencarian "{{ $search }}"
                    @else
                      Tidak ada produk dalam kategori {{ ucfirst($kategori) }}
                    @endif
                  </td>
                </tr>
                @endforelse

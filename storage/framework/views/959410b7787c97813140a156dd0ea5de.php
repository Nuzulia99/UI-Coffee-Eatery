      ...
      <div class="items-center block w-auto overflow-hidden">
        <ul class="flex flex-col pl-0 mb-0">
          <li class="mt-0.5 w-full">
            <a class=" dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors" href="<?php echo e(route('dashboard')); ?>">
          ...
          <li class="mt-0.5 w-full">
            <a class="py-2.7 bg-slate-500/13 dark:text-white dark:opacity-80 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 font-semibold text-slate-700 transition-colors" href="<?php echo e(route('produk.index')); ?>">
              <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
        ...
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal text-white before:float-left before:pr-2 before:text-white before:content-['/']" aria-current="page">Edit Barang</li>
            </ol>
            <h6 class="mb-0 font-bold text-white capitalize">Edit Barang</h6>
          </nav>
        </div>
      </nav>
      ...
      <div class="w-full px-6 py-6 mx-auto">
        <div class="relative flex flex-col min-w-0 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="capitalize dark:text-white">Form Edit Barang</h6>
          </div>
          <div class="p-6">
            <?php if($errors->any()): ?>
            ...
            <form action="<?php echo e(route('produk.update', $produk->ProdukID)); ?>" method="POST" enctype="multipart/form-data">
              <?php echo csrf_field(); ?>
              <?php echo method_field('PUT'); ?>
              <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-2">Nama Barang</label>
                <input type="text" name="NamaProduk" value="<?php echo e($produk->NamaProduk); ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
              </div>
              <div class="mb-4">
                ...
                <select name="Kategori" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
                  ...
                  <option value="Drinks" <?php echo e($produk->Kategori == 'Drinks' ? 'selected' : ''); ?>>Drinks</option>
                  <option value="Main Course" <?php echo e($produk->Kategori == 'Main Course' ? 'selected' : ''); ?>>Main Course</option>
                  <option value="Snack" <?php echo e($produk->Kategori == 'Snack' ? 'selected' : ''); ?>>Snack</option>
                  <option value="Dessert" <?php echo e($produk->Kategori == 'Dessert' ? 'selected' : ''); ?>>Dessert</option>
                </select>
              </div>
              <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-2">Gambar</label>
                <div class="mb-3">
                  <?php if($produk->Gambar): ?>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mb-2">Gambar saat ini:</p>
                    <img src="<?php echo e(asset($produk->Gambar)); ?>" alt="<?php echo e($produk->NamaProduk); ?>" class="h-32 w-32 object-cover rounded-lg mb-3">
                  <?php endif; ?>
                </div>
                <input type="file" name="Gambar" accept="image/*" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600">
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Format: JPG, PNG, GIF (Max: 2MB)</p>
              </div>
              <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-2">Harga</label>
                <input type="number" name="Harga" step="0.01" value="<?php echo e($produk->Harga); ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
              </div>
              <div class="mb-4">
                <label class="block text-sm font-semibold text-slate-700 dark:text-white mb-2">Stok</label>
                <input type="number" name="Stok" value="<?php echo e($produk->Stok); ?>" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-800 dark:text-white dark:border-slate-600" required>
              </div>
              <div class="flex gap-2">
                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg">
                  <i class="fas fa-save"></i> Simpan
                </button>
                <a href="<?php echo e(route('produk.index')); ?>" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg">
...
<?php /**PATH C:\laragon\www\coffee\resources\views/produk/edit.blade.php ENDPATH**/ ?>
      ...
      <div class="w-full px-6 py-6 mx-auto">
                  ...
                  <div class="flex-none w-2/3 max-w-full px-3">
                    <div>
                      ...
                      <p class="mb-0 dark:text-white dark:opacity-60">
                        <span class="text-lg font-bold leading-normal text-gray-500" id="totalPenjualanValue">Rp <?php echo e(number_format($totalPenjualanHariIni, 0, ',', '.')); ?></span>
                      </p>
                      <p class="text-xs text-gray-500 dark:text-white/60 mt-1" id="jumlahTransaksiValue"><?php echo e($jumlahTransaksiHariIni); ?> transaksi</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <p class="mb-0 dark:text-white dark:opacity-60">
                        <?php if($produkTerlaris): ?>
                          <span class="text-sm font-bold leading-normal text-gray-700" id="produkTerlariValue"><?php echo e($produkTerlaris->NamaProduk); ?></span>
                        <?php else: ?>
                      ...
                      <p class="text-xs text-gray-500 dark:text-white/60 mt-1" id="produkTerjualValue">
                        <?php if($produkTerlaris): ?>
                          <?php echo e($produkTerlaris->total_terjual); ?> item terjual
                        <?php else: ?>
                          Tidak ada penjualan
                        <?php endif; ?>
                      </p>
          ...
          <div class="w-full max-w-full px-3 lg:w-5/12 lg:flex-none">
            <div style="border: 2px solid #3d5a73; border-radius: 1rem; display: flex; overflow: hidden; pointer-events: auto; position: relative; height: 100%;">
              ...
              <div class="absolute inset-0 w-full h-full">
                <img class="w-full h-full object-cover" src="<?php echo e(asset('assets/img/COFFEE & EATERY_20260114_122950_0000.png')); ?>" alt="UI Coffee & Eatery" />
          ...
          <div class="overflow-x-auto">
            <table id="transactionTable" class="items-center w-full mb-0 align-top border-collapse dark:border-white/40">
              ...
              <tbody id="transactionTableBody">
                <?php $__empty_1 = true; $__currentLoopData = $transaksiTerbaru; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                  <td class="px-6 py-5 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white text-slate-700"><?php echo e($key + 1); ?></p>
                  </td>
                  <td class="px-6 py-5 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white text-slate-700"><?php echo e($item->TanggalPenjualan); ?></p>
                  </td>
                  <td class="px-6 py-5 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white text-slate-700"><?php echo e($item->pelanggan->NamaPelanggan); ?></p>
                  </td>
                  <td class="px-6 py-5 whitespace-nowrap text-center">
                    <p class="mb-0 text-sm font-semibold leading-tight dark:text-white text-slate-700">Rp <?php echo e(number_format($item->TotalHarga, 0, ',', '.')); ?></p>
                  </td>
                  <td class="px-6 py-5 whitespace-nowrap text-center">
                    <a href="<?php echo e(route('penjualan.receipt', $item->PenjualanID)); ?>" class="inline-block text-white font-semibold py-2 px-4 rounded transition hover:shadow-lg" style="background-color: #3d5a73;">
                      Lihat
                    </a>
                  </td>
                </tr>
        ...
        <script>
          ...
          async function loadTodayTransactions() {
            try {
              const response = await fetch('<?php echo e(route("api.today-transactions")); ?>');
              ...
              data.transaksi.forEach(item => {
                html += `
                  <tr class="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                    ...
                    <td class="px-6 py-5 whitespace-nowrap text-center">
                      <a href="<?php echo e(route('penjualan.receipt', '')); ?>/${item.id}" class="inline-block text-white font-semibold py-2 px-4 rounded transition hover:shadow-lg" style="background-color: #3d5a73;">
                        Lihat
                      </a>
                    </td>
                  </tr>
          ...
          async function loadPetugasStats() {
            try {
              const response = await fetch('<?php echo e(route("api.petugas-stats")); ?>');
              const data = await response.json();
        ...
        <script>
          ...
          async function loadPetugasDailySalesChart() {
            try {
              const response = await fetch('<?php echo e(route("api.petugas-daily-sales")); ?>');
              const data = await response.json();
...
<?php /**PATH C:\laragon\www\coffee\resources\views/dashboard-petugas.blade.php ENDPATH**/ ?>